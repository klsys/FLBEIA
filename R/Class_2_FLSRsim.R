...
FLSRsim <- function(...){
    ...
    if('model' %in% slots){
	 ...
	 nmparams1 <-  nmparams2 <- NULL     
        if(!all(grepl('~', x[['model']]))) nmparams1 <- unique(unlist(lapply(x[['model']][!grepl('~', x[['model']])], function(m)all.vars(get(m)()[[2]]))))
	if(any(grepl('~', x[['model']]))) nmparams2 <- unique(unlist(lapply(x[['model']][grepl('~', x[['model']])],function(x)all.vars(formula(x)))[[3]]))
        # nmparams <- all.vars(get(x[['model']], pos = 2)()[[2]])
        nmparams <- unique(c(nmparams1,nmparams2))
...
SRsim <- function(object, year = 1, season = 1, iter = 'all')  # year and season either numeric (position) or character (name)
  {
    ...
    if(length(grep('~', object@model)) == 0)
        ...
        model <- unlist(lapply(object@model,formula))
